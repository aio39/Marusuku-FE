...
const BasicWrapper: FC<BasicP> = memo(
  ({ registerReturn, error, data: [label, placeholder], isNotRequired = false, children }) => {
...
const InputWrapper: FC<InputP> = memo(
  ({ registerReturn, error, data, isNotRequired = false, inputP, children }) => {
    const name = registerReturn.name
...
const SelectWrapper: FC<SelectP> = memo(
  ({ registerReturn, error, data, selectList: dataList, isNotRequired = false, children }) => {
    const name = registerReturn.name
    const [label, placeholder] = data
    return (
      <BasicWrapper
...
const NumberInputWrapper: FC<NumberP> = memo(
  ({ registerReturn, error, data, isNotRequired = false, defaultValue, min, max, children }) => {
    const name = registerReturn.name
    const [label, placeholder] = data
    return (
      <BasicWrapper
