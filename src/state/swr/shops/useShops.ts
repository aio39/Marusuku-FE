...
type IUseShopsInfinite = {
  (searchQuery: string, query?: CommonFSW, news?: NEWS, isPass?: boolean): SWRInfiniteResponse<
    Pagination<Shop>,
    any
  >
  (
    searchQuery: string,
    query?: CommonFSW,
    distance?: DistanceSearchData,
    isPass?: boolean
  ): SWRInfiniteResponse<Pagination<Shop>, any>
}
...
const useShopsInfinite: IUseShopsInfinite = (searchQuery, query, condition, isPass) => {
  const getKey: KeyLoader<Pagination<Shop>> = (pageIndex, previousPageData) => {
